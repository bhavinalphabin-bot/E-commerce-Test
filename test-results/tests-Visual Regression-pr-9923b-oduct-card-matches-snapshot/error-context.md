...
- Name: tests/Visual Regression/product-card.spec.ts >> product card matches snapshot
- Location: tests/Visual Regression/product-card.spec.ts:3:5
...
```ts
  ...
  2  | 
  3  | test('product card matches snapshot', async ({ page }) => {
  4  |   await page.goto('https://automationexercise.com/products');
  5  | 
  6  |   const card = page.locator('.product-image-wrapper').first();
  7  | 
> 8  |   await expect(card).toHaveScreenshot({
     |                      ^ Error: expect(locator).toHaveScreenshot(expected) failed
  9  |     maxDiffPixels: 50
  10 |   });
  11 | });
