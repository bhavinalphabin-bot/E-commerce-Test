...
import { AUDIT_CATEGORY_CODES, isRecognizedAuditCategory, normalizeAuditCategory, normalizeAuditSubCategory, } from "./audit-categories.js";
import { findingItemSchema } from "../../lib/item-schemas.js";
const LEADING_PROGRESS_LINE_PATTERN = /^\*\*(?:reading|scanning|building|analy(?:s|z)ing|reviewing|inspecting|writing|generating)\b[^*]*\*\*$/i;
const AUDIT_CATEGORY_LIST = AUDIT_CATEGORY_CODES.join(", ");
export const testAuditTool = {
    name: "test_audit",
    description: "TestDino-backed Playwright test quality audit. This tool is a NAMED CAPABILITY, not a style of output — when the user invokes it, you MUST call this tool, not simulate it in chat. " +
    ...
    inputSchema: {
        ...
        properties: {
            ...
            action: {
                ...
                description: "Action to perform. 'analyze' fetches the audit context (prompt + signals) — after analyzing, call this tool again with the report fields to submit. 'list' shows past reports. 'get' retrieves a specific report.",
            },
            branch: {
                type: "string",
                description: "Git branch to scope the audit to. Optional — if omitted and not auto-detected from git, the audit still proceeds without a branch. Do not prompt the user for a branch; just call the tool without it. Also acts as a filter for 'list'.",
            },
            scope: {
                type: "string",
                enum: ["testcase", "feature", "spec_file", "suite"],
                description: "Audit scope. Defaults to 'suite'. Use 'feature' or 'spec_file' when the user names a slice such as auth/login, dashboards, or a specific spec.",
            },
            target: {
                type: "object",
                description: "Structured audit target, such as spec path or feature area. Pass this when the user asks for a scoped audit so the report stays centered on that slice.",
                additionalProperties: true,
            },
            reportName: {
                type: "string",
                description: "A short, human-readable name for the audit report (e.g. 'Login Flow Tests', 'API Integration Suite'). Displayed as the report title in the dashboard. Include when submitting a completed report.",
            },
            score: {
                type: "number",
                minimum: 0,
                maximum: 100,
                description: "Audit score (0-100). Include when submitting a completed report.",
            },
            findings: {
                type: "array",
            ...
            markdownReport: {
                type: "string",
                description: "The complete markdown report content. Prefer using markdownReportPath instead to avoid large tool calls.",
            },
            markdownReportPath: {
                type: "string",
                description: "Path to a local markdown file to read for submission. Use a relative path from the workspace (see TESTDINO_MCP_WORKSPACE), or an absolute path. Preferred over markdownReport.",
            },
            reportId: {
                type: "string",
            ...
            outputPath: {
                type: "string",
                description: "Relative file path for writing the report; resolved from TESTDINO_MCP_WORKSPACE if set, else process.cwd(). Defaults to TEST-AUDIT.md.",
            },
        },
...
export const getAuditReportTool = {
    ...
    description: "Read-only TestDino Playwright audit reads. Three modes via action: action='context' fetches the server-curated audit prompt + branch signals to START an audit; action='list' browses previously submitted reports; action='get' retrieves one saved report by reportId. " +
    ...
    inputSchema: {
        ...
        properties: {
            ...
            action: {
                ...
                description: "Read mode: 'context' fetches audit prompt + branch signals, 'list' browses reports, 'get' retrieves one report by reportId.",
            },
            branch: {
                type: "string",
                description: "Git branch. For action='context', the branch to audit when known. For action='list', optional filter.",
            },
            reportId: {
                type: "string",
            ...
            writeMarkdown: {
                type: "boolean",
                description: "When action='get', write the returned markdown report to a local file.",
            },
            outputPath: {
                type: "string",
                description: "Relative file path for writing the report. Defaults to TEST-AUDIT.md.",
            },
        },
        required: ["projectId", "action"],
    },
...
export const submitAuditReportTool = {
    ...
    description: "Final step of the TestDino Playwright audit flow. Submit a completed audit report after get_audit_report(action='context') and local analysis. Requires score and markdownReport or markdownReportPath.",
    inputSchema: {
        type: "object",
        properties: {
            ...
            markdownReportPath: {
                type: "string",
                description: "Path to a local markdown report file. Preferred over markdownReport.",
            },
            writeMarkdown: {
                type: "boolean",
                description: "When true and markdownReport is provided inline, also save a local copy.",
            },
            outputPath: {
                type: "string",
...
function assertWithinWorkspaceBase(fieldName, requestedPath, resolvedPath, workspaceBase) {
    const relativePath = relative(workspaceBase, resolvedPath);
    if (relativePath === ".." ||
        relativePath.startsWith(`..${sep}`) ||
        isAbsolute(relativePath)) {
        throw new Error(`${fieldName} resolves outside workspace base: '${requestedPath}'`);
    }
}
function resolveWorkspacePath(fieldName, requestedPath) {
    ...
    if (isAbsolute(trimmedPath)) {
        throw new Error(`${fieldName} must be a relative path, got: '${trimmedPath}'`);
    }
    const resolvedPath = resolve(workspaceBase, trimmedPath);
    assertWithinWorkspaceBase(fieldName, trimmedPath, resolvedPath, workspaceBase);
    return resolvedPath;
}
async function resolveMarkdownReportPath(requestedPath) {
    const trimmedPath = requestedPath.trim();
    const workspaceBase = getWorkspaceBase();
    if (!trimmedPath) {
    ...
    if (!isAbsolute(trimmedPath)) {
        assertWithinWorkspaceBase("markdownReportPath", trimmedPath, resolvedPath, workspaceBase);
    }
    let fileStats;
    try {
        ...
        const errorCode = error && typeof error === "object" && "code" in error
            ...
            : "";
        if (errorCode === "ENOENT") {
            throw new Error(`markdownReportPath not found: ${resolvedPath}. Set TESTDINO_MCP_WORKSPACE to your project root for relative paths, or pass an absolute path to the file.`);
        }
        throw error;
    }
    if (!fileStats.isFile()) {
...
async function writeLocalReport(markdownReport, outputPath) {
    const content = markdownReport.trim();
    if (!content) {
        return { saved: false, reason: "markdown_report_empty" };
    }
    const requestedPath = typeof outputPath === "string" && outputPath.trim()
        ? outputPath.trim()
        : "TEST-AUDIT.md";
...
function normalizeMarkdownReport(markdownReport) {
    ...
    while (scanIndex < cleanedLines.length &&
...
function sanitizeAuditFindings(findings) {
    return findings.map((finding, index) => {
        ...
        if (rawCategory != null &&
            ...
            throw new Error(`findings[${index}].category '${rawCategory}' is invalid. Use one of: ${AUDIT_CATEGORY_LIST}`);
        }
        const normalizedCategory = normalizeAuditCategory(rawCategory);
        const normalizedSubCategory = normalizeAuditSubCategory(finding.subCategory);
        return {
            ...finding,
...
function requireAuditToken(args) {
...
function auditTextResponse(response) {
    return {
        content: [
            {
                type: "text",
...
export async function handleGetAuditReport(args) {
    ...
    const validActions = ["context", "list", "get"];
    if (!validActions.includes(action)) {
        throw new Error(`Invalid action '${args.action}'. Must be one of: ${validActions.join(", ")}`);
    }
    const explicitBranch = typeof args.branch === "string" && args.branch.trim()
        ? args.branch.trim()
        : undefined;
    ...
    try {
        ...
        if (action === "context") {
            ...
            const markdown = typeof reportData?.markdownReport === "string"
            ...
            if (args.writeMarkdown && markdown) {
                try {
                    ...
                    response = {
                        ...
                        localReportWrite: {
                            saved: false,
                            reason: writeError instanceof Error
                                ? writeError.message
                                : String(writeError),
                        },
                    };
        ...
        throw new Error(`Failed to get audit report: ${normalizeAuditErrorMessage(errorMessage)}`);
    }
}
export async function handleSubmitAuditReport(args) {
    const token = requireAuditToken(args);
    if (!args?.projectId) {
        throw new Error("projectId is required");
    }
    ...
    const normalizedMarkdownReport = typeof markdownReport === "string"
    ...
    try {
        ...
        const hasExplicitTarget = args.target &&
        ...
        if (args.writeMarkdown &&
            ...
            try {
                const writeResult = await writeLocalReport(normalizedMarkdownReport, args.outputPath);
                response = {
                    ...response,
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        throw new Error(`Failed to submit audit report: ${normalizeAuditErrorMessage(errorMessage)}`);
    }
}
export async function handleTestAudit(args) {
    const token = requireAuditToken(args);
    if (!args?.projectId) {
        throw new Error("projectId is required");
    }
    ...
    const VALID_ACTIONS = ["analyze", "list", "get"];
    if (!VALID_ACTIONS.includes(String(args.action))) {
        throw new Error(`Invalid action '${args.action}'. Must be one of: ${VALID_ACTIONS.join(", ")}`);
    }
    const action = String(args.action);
    const explicitBranch = typeof args.branch === "string" && args.branch.trim()
        ? args.branch.trim()
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        throw new Error(`Failed to run test audit: ${normalizeAuditErrorMessage(errorMessage)}`);
    }
}
