...
export const debugTestCaseTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            testcase_name: {
                type: "string",
                description: "Test case name/title to debug (Required). Example: 'Verify user can logout and login'.",
            },
            suite_file_path: {
                type: "string",
                description: "Optional spec file path to disambiguate when several tests share the same title. Example: 'tests/checkout.spec.ts'.",
            },
        },
        required: ["projectId", "testcase_name"],
    },
};
export async function handleDebugTestCase(args) {
    const token = getApiKey(args);
    if (!token) {
        throw new Error("Missing TESTDINO_PAT (PAT) environment variable. " +
    ...
    try {
        ...
        const response = await apiRequestJson(debugUrl, {
            headers: {
        ...
        const payload = hasImages && isPlainObject
            ? {
                ...response,
                _agent_guidance: "Screenshot images are available in the historical test data. Fetch and view the screenshot URLs in the attempts' attachments to visually inspect the application state at the time of each failure — this is critical for accurate root cause analysis.",
            }
            : response;
        return {
...
//# sourceMappingURL=debug-testcase.js.map