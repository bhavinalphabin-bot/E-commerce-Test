...
export const getTestCaseDetailsTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            testcase_name: {
                type: "string",
                description: "Test case title (partial match, case-insensitive). Must be combined with testrun_id when used alone. Example: 'Verify user can logout and login'.",
            },
            testrun_id: {
                type: "string",
                description: "Single test run ID to filter results. Example: 'test_run_6901b2abc6b187e63f536a6b'.",
            },
            by_fulltitle: {
                type: "string",
            ...
            include_history: {
                type: "boolean",
                description: "Include historical executions of the same test case when searching by name. Default: false.",
                default: false,
            },
            history_limit: {
                type: "number",
                description: "Max number of history entries to return (max: 100, default: 10).",
                default: 10,
            },
            steps_filter: {
                type: "string",
                enum: ["failed_only"],
                description: "Filter steps in each attempt. Use 'failed_only' to return only steps with errors, stripping passing setup and hook steps.",
            },
        },
        required: ["projectId"],
    },
...
export async function handleGetTestCaseDetails(args) {
    ...
    const hasSearchParam = !!(args?.testcase_name ||
        args?.by_title ||
        args?.by_fulltitle);
    if (!hasTestCaseId && !hasSearchParam) {
        throw new Error("At least one of the following must be provided: 'testcase_id', 'testcase_name', or 'by_fulltitle'.");
    }
    try {
        ...
        const hasImages = probe.includes('"contentType": "image/') ||
            probe.includes('"screenshots"');
        const isPlainObject = response !== null &&
            typeof response === "object" &&
            !Array.isArray(response);
        const payload = hasImages && isPlainObject
            ? {
                ...response,
                _agent_guidance: "This test case has screenshot images attached. Fetch and view the screenshot URLs (in the 'screenshots' or attachment 'path' fields) to visually inspect the application state at the time of failure — this is critical for accurate diagnosis.",
            }
            : response;
        return {
