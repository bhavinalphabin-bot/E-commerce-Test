...
export const listTestCasesTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            by_testrun_id: {
                type: "string",
                description: "Test run ID(s). Single ID or comma-separated for multiple runs (max 20). Example: 'test_run_123' or 'run1,run2,run3'. Not required when using a cross-run filter (by_branch, by_commit, by_author, by_environment, by_time_interval, by_pages).",
            },
            counter: {
                type: ["number", "string"],
                description: "Test run counter number. Alternative to by_testrun_id. Not required when using a cross-run filter (by_branch, by_commit, by_author, by_environment, by_time_interval, by_pages). Example: 43.",
            },
            by_status: {
                type: "string",
                description: "Filter by status: 'passed', 'failed', 'flaky', 'skipped', 'interrupted', 'incomplete', or 'running'. (ID/Counter is required while using this parameter)",
                enum: [
                    "passed",
                    "failed",
                    "flaky",
            ...
            by_total_runtime: {
                type: "string",
                description: "Per-test duration filter. Numbers are SECONDS by default; suffix with `ms` for milliseconds or `s` for seconds. Examples: '>10', '<1000ms', '>5s'. (ID/Counter or a run scope is required while using this parameter)",
            },
            by_artifacts: {
                type: "boolean",
            ...
            by_attempt_number: {
                type: "number",
                description: "Exact retry count filter. 0 = initial/no-retry (attempt_count=1), 1 = one retry (attempt_count=2). (ID/Counter is required while using this parameter)",
            },
            by_pages: {
                type: "number",
            ...
            limit: {
                type: "number",
                description: "Test cases per page within the resolved run(s). Snapped to the nearest of 10, 25, 50, 100 (data-handler's allowed page sizes) — other values are adjusted, not rejected. Requires a run scope (by_testrun_id, counter, or a cross-run filter); it does not resolve runs on its own.",
            },
            by_environment: {
                type: "string",
            ...
            page: {
                type: "number",
                description: "1-indexed page number for pagination within the resolved run(s) (default: 1). Requires a run scope (by_testrun_id, counter, or a cross-run filter). To page across runs, use by_pages.",
                default: 1,
            },
        },
        required: ["projectId"],
...
export async function handleListTestCases(args) {
    ...
    const hasTestRunFilters = !!(args?.by_branch ||
        args?.by_commit ||
        args?.by_author ||
        args?.by_environment ||
    ...
    if (!hasTestRunId && !hasCounter && !hasTestRunFilters) {
        throw new Error("A run scope is required: provide by_testrun_id, counter, or a cross-run filter (by_branch, by_commit, by_author, by_environment, by_time_interval, by_pages). page/limit paginate within a run scope — they do not select runs on their own.");
    }
    try {
        const params = {
