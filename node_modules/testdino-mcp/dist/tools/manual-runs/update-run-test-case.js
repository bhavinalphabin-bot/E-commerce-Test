...
export const updateRunTestCaseTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            rtcRef: {
                type: "string",
                description: "Per-case record reference — `tcm_rtc_...` _id, caseKey ('TC-156'), or underlying test case _id (required).",
            },
            updates: {
                type: "object",
                description: "Fields to update. Quick verdict fields: assigneeUserId (email or _id, pass null to unassign), result/status (display or canonical form), elapsed (seconds). Detailed-mode fields: comment (HTML), linkedIssues, attachments, stepResults (e.g. [{order, status, comment}]). Quick and detailed are mutually exclusive — mixing an assignee with detailed fields is rejected server-side.",
            },
        },
        required: ["projectId", "runId", "rtcRef", "updates"],
    },
};
export async function handleUpdateRunTestCase(args) {
    const token = getApiKey(args);
    if (!token) {
        throw new Error("Missing TESTDINO_PAT environment variable. Configure it in your .cursor/mcp.json under 'env'.");
    }
    if (!args?.projectId)
    ...
    if (!args?.rtcRef)
        throw new Error("rtcRef is required");
    if (!args?.updates || typeof args.updates !== "object") {
        throw new Error("updates must be an object containing fields to modify");
    }
    try {
        const url = endpoints.updateRunTestCase(String(args.projectId), String(args.runId), String(args.rtcRef));
...
//# sourceMappingURL=update-run-test-case.js.map