...
```mermaid
graph LR
    A[Playwright] -->|test events| B["@testdino/playwright"]
    B -->|stream| C[TestDino Platform]
...
**2.0 is a breaking change.** The artifact upload pipeline moved to a new server contract; the wire format for `test:end` attachments flipped.

- **Upgrade in lockstep with your TestDino ingestion service.** The 2.0 CLI against pre-cutover ingestion will silently drop attachments (ingestion discards the new `blobKey` field). Pin your CI to `@testdino/playwright@^1` until your platform is cut over.
- **Old CLI versions continue to work but their artifacts silently won't attach after the server cutover** — post-cutover ingestion discards the deprecated `path` field. Any 1.x CLI still in your fleet must be upgraded, or its dashboards will show no artifacts.
- **Attachment states are now explicit.** In the dashboard, a failed / disabled / disallowed upload appears as **`unavailable`** rather than a broken link. Local file paths are never streamed (privacy fix).
- **Configuration and CLI flags are unchanged** — no config migration.
...
Copyright 2026 TestDino. All rights reserved.
