...
var EventBuffer = class {
  ...
  async add(event) {
    ...
    if (this.events.length >= this.maxSize) await this.flush();
  }
  async flush() {
...
var GitMetadataCollector = class extends BaseMetadataCollector {
  ...
  async applyGitHubContext(base2) {
    ...
    resolved.pr = this.extractPRMetadata(eventData);
    const headRef = process.env.GITHUB_HEAD_REF;
    if (this.isNonEmptyString(headRef)) {
      resolved.branch = headRef;
    }
...
var ArtifactUploadClient = class {
  ...
  async doPut(uploadUrl, body, contentType) {
    await import_axios3.default.put(uploadUrl, body, {
      headers: {
...
var TestdinoReporter = class _TestdinoReporter {
  ...
  loadCliConfig() {
    ...
    try {
      if (!(0, import_fs.existsSync)(cliConfigPath)) {
        return {};
      }
      const configContent = (0, import_fs.readFileSync)(cliConfigPath, "utf-8");
  ...
  filterUploadable(stated) {
    ...
    for (const item of stated) {
      ...
      if (isAllowedExtension((0, import_path4.basename)(item.path)) && isWithinSizeLimit(item.sizeBytes)) {
  ...
  async putBatch(uploadable, testId, client) {
    ...
    try {
      const request = ready.map((f) => ({
        ...
        name: (0, import_path4.basename)(f.path),
        testId,
