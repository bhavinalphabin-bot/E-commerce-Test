...
var HttpClient = class {
  ...
  /**
   * Authenticate with the data-handler and receive Kafka connection config.
   * Contract: POST /api/v1/reporter/auth with Bearer td_api_<hex>.
   */
  async authenticate() {
    try {
      ...
      if (import_axios.default.isAxiosError(error) && error.response) {
        ...
        if (status === 403) {
          const data = error.response.data;
          if (data?.code === "TOKEN_TYPE_NOT_ALLOWED") {
            throw new InvalidTokenTypeError(data.message);
          }
          throw new ForbiddenAuthError(data?.message);
        }
        if (status === 405) {
          throw new ServerEndpointError(AUTH_PATH);
        }
      }
  ...
  /**
   * Send events via HTTP POST to the server-provided fallback URL
   */
...
var KafkaEventProducer = class {
  ...
  async send(event) {
    ...
    try {
      ...
      const orphanedProducer = this.producer;
      this.producer = null;
      orphanedProducer?.disconnect().catch(() => {
      });
      throw error;
    }
  }
   ...
   *
   * On error: resets `isConnected` to false and nulls the producer reference
   * before rethrowing. This signals the delivery manager's retry loop to
   * call `connect()` on the next attempt, which creates a fresh producer.
   */
...
var EventBuffer = class {
   ...
   * Does NOT flush remaining events — caller is responsible for a final explicit flush.
   */
  destroy() {
...
var EventDeliveryManager = class {
  ...
  httpSuccessCount = 0;
  /** Timestamp of the last successful event delivery (Kafka or HTTP) */
  lastDeliveryAt = 0;
  constructor(options) {
  ...
  async tryHttpWithRetry(events) {
    ...
    for (let attempt = 1; attempt <= this.maxAttempts; attempt++) {
      try {
        ...
        if (import_axios2.default.isAxiosError(error) && error.response?.status === 413) {
          this.logger.error("HTTP batch too large (413) \u2014 events discarded");
          throw error;
        }
        if (import_axios2.default.isAxiosError(error) && error.response?.status === 401) {
          this.logger.error("HTTP fallback auth failed (401) \u2014 check API key");
          throw error;
        }
  ...
  /**
   * Attempt to reconnect to Kafka after sustained HTTP success.
   * Resets counter regardless of outcome to avoid constant re-probing.
   */
...
var GitMetadataCollector = class extends BaseMetadataCollector {
  ...
  async collectMetadata() {
    ...
    const commitInfo = extractedValues[2];
    let message = commitInfo?.message;
    let author = commitInfo?.author;
    let email = commitInfo?.email;
    let timestamp = commitInfo?.timestamp;
    const repoUrl = extractedValues[3];
    const isDirty = extractedValues[4];
    let prMetadata;
  ...
  async configureGitForCI() {
    ...
    try {
      ...
      const { stdout } = await execa("git", ["config", "--global", "--get-all", "safe.directory"], {
        timeout: this.options.timeout,
        reject: false
      });
      const existing = stdout.split("\n").map((line) => line.trim());
      if (existing.includes(this.options.cwd) || existing.includes("*")) return;
...
var ArtifactUploader = class {
  ...
  async uploadFile(attachment, testId) {
    const startTime = Date.now();
    const fileName = (0, import_path3.basename)(attachment.path);
    try {
      const stats = await (0, import_promises3.stat)(attachment.path);
      ...
      const extension = (0, import_path3.extname)(fileName).slice(1).toLowerCase();
      ...
      await this.uploadWithRetry(attachment.path, uploadUrl, attachment.contentType, fileSize);
   ...
   */
  async uploadWithRetry(filePath, uploadUrl, contentType, fileSize) {
    let lastError = null;
    for (let attempt = 0; attempt < this.options.maxAttempts; attempt++) {
      try {
        await this.doUpload(filePath, uploadUrl, contentType, fileSize);
        return;
      } catch (error) {
        lastError = error instanceof Error ? error : new Error(String(error));
        if (attempt < this.options.maxAttempts - 1) {
   ...
   */
  async doUpload(filePath, uploadUrl, contentType, fileSize) {
    const fileStream = (0, import_fs.createReadStream)(filePath);
    try {
      await import_axios4.default.put(uploadUrl, fileStream, {
        headers: {
...
async function generateIstanbulHtmlReport(coverageMerger, options) {
  await (0, import_promises4.rm)(options.outputDir, { recursive: true, force: true }).catch(() => {
  });
...
function createBoxHelpers() {
  ...
  const row = (content) => `  ${colors.dim("\u2502")} ${pad(content, BOX_WIDTH - 1)}${colors.dim("\u2502")}`;
...
var TestdinoReporter = class _TestdinoReporter {
  ...
  // Delivery failure tracking for buffer event restoration
  ...
  // Heartbeat monitor — sends lightweight events during silence to prevent TTL eviction
  heartbeatTimer = null;
  ...
  async performAsyncInit(config, suite, token) {
    ...
    try {
      ...
      if (this.artifactsEnabled) {
        try {
          const sasClient = new SASTokenClient({ token, serverUrl: this.getBaseServerUrl() });
  ...
  async sendEvents(events) {
    ...
    if (this.deliveryManager) {
      try {
        ...
        const status = error?.response ? error.response?.status : void 0;
        if (status === 413 || status === 401) {
          this.log.error(`Permanent delivery error (HTTP ${status}) \u2014 discarding ${events.length} events`);
          return;
        }
  ...
  /**
   * Get base server URL (origin only, no path suffix).
   * HTTP and SAS clients append their own paths.
   */
  getBaseServerUrl() {
    return this.config.serverUrl || process.env.TESTDINO_SERVER_URL || "https://api.testdino.com";
  }
  /**
  ...
  /**
   * Normalize an absolute file path to repo-relative using rootDir.
   * Returns the path unchanged if rootDir is not set or path is outside rootDir.
   */
  ...
  /**
   * Start heartbeat monitor — sends run:heartbeat during silence periods.
   * Only called after init succeeds and delivery manager exists.
   */
  ...
  /**
   * Stop heartbeat monitor and clear timer
   */
  ...
  /**
   * Compress a coverage event if its JSON payload exceeds the 1MB threshold.
   * Returns a CompressedCoverageDataEvent envelope when compressed, or the
   * original event unchanged when under the threshold or if compression fails.
   */
