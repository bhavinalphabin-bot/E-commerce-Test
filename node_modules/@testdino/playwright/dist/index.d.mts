 ...
 * HTTP 403 TOKEN_TYPE_NOT_ALLOWED — the provided bearer is a PAT (td_pat_) or cookie JWT
 * rather than a pipeline API key (td_api_). Actionable so the user knows which token to use.
 */
declare class InvalidTokenTypeError extends TestDinoServerError {
    constructor(message?: string);
}
/**
 * Authentication forbidden by server (HTTP 403, non-token-type cause).
 * Distinct from InvalidTokenTypeError which is a known 403 sub-case.
 */
declare class ForbiddenAuthError extends TestDinoServerError {
    constructor(message?: string);
}
/**
 * Server rejected the auth endpoint with 405 Method Not Allowed.
 * Usually means serverUrl points at the wrong service.
 */
 ...
 */
type ServerError = QuotaExhaustedError | QuotaExceededError | InvalidTokenTypeError | ForbiddenAuthError | ServerEndpointError;
...
/**
 * Lightweight heartbeat event sent during silence periods to prevent
 * TTL-based eviction of active runs across the pipeline.
 * Intentionally has no sequence number — not part of event ordering.
 */
...
/**
 * Compressed coverage data event — sent instead of CoverageDataEvent when
 * the JSON payload exceeds 1MB. The full event is gzip-compressed and
 * base64-encoded into the `payload` field.
 *
 * Ingestion detects compression via the `compression` field and decodes with:
